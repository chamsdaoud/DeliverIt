...
import { PackageIcon, UserIcon, CashIcon, CardIcon, CheckIcon, XIcon, PlusIcon, SearchIcon } from '../Icons'
...
export default function ParcelList({ parcels, loading, onAssign, onRefresh, onCreateNew }) {
  ...
  const handleValidationSuccess = (result) => {
    ...
    onRefresh()
    if (result === 'validated') alert('Parcel validated successfully. Status set to Registered.')
    if (result === 'rejected')  alert('Parcel rejected. Status set to Failed.')
  }
